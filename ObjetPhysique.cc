...
: ObjetMobile(e, e_prime),contrainte(c) , champ_f(champ_f), m(m), charge(charge), dim(dim)
{
	if (contrainte == nullptr) {
		contrainte = new Libre;
	}
	if (champ_f == nullptr) {
		champ_f = new Vide;
	}
}
ObjetPhysique::ObjetPhysique(const Vecteur &e, const Vecteur &e_prime, double m, double charge, double dim)
: ObjetPhysique(e, e_prime, new Libre, new Vide, m, charge, dim)
{}
...
ostream& ObjetPhysique::affiche(ostream& sortie) const {
	...
	sortie << m << " # masse" << endl;
	sortie << "contrainte : " << *contrainte << endl;
	return sortie;
}

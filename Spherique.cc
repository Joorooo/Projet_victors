...
//methodes privées
double Spherique::get_teta(const ObjetPhysique &p) const {
	Vecteur e1({1, 0});
	return p.get_E()*e1;
}
double Spherique::get_phi(const ObjetPhysique &p) const {
	Vecteur e2({0, 1});
	return p.get_E()*e2;
}
double Spherique::get_teta_prime(const ObjetPhysique &p) const {
	Vecteur e1({1, 0});
	return p.get_E_prime()*e1;
}
double Spherique::get_phi_prime(const ObjetPhysique &p) const {
	Vecteur e2({0, 1});
	return p.get_E_prime()*e2;
}
...
Vecteur Spherique::applique_force(const ObjetPhysique &p, const Vecteur &force, double t) const {
	if (p.get_m() == 0) {
		...
		Vecteur teta_carth({cos(teta)*cos(phi), cos(teta)*sin(phi), -sin(teta)});
		Vecteur phi_carth({-sin(phi), cos(phi), 0});
		
		double force_teta = -1*force*teta_carth;
		double force_phi = force*phi_carth;
		
		double teta_pprime = (1/(p.get_m()*r))*force_teta + sin(teta)*cos(teta)*phi_prime*phi_prime;
		double phi_pprime(0.0);
		if (fabs(teta) >= 1E-2) {
			phi_pprime = (1/(p.get_m()*r*sin(teta)))*force_phi - 2*(1/tan(teta))*teta_prime*phi_prime;
		}

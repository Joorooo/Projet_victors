...
Vecteur Spherique::applique_force(const ObjetPhysique &p, const Vecteur &force, double t) const {
	if (p.get_m() <= limite) {
		...
		double force_teta = -1*force*teta_carth; //vecteur force selon -teta_carth
		double force_phi = force*phi_carth; //vectuer force selon phi_carth
		
		double teta_pprime = (1.0/(p.get_m()*r))*force_teta + sin(teta)*cos(teta)*phi_prime*phi_prime; //double dérivée de l'angle teta
		double phi_pprime(0.0);
		if (fabs(teta) >= 1E-2) {
			phi_pprime = (1.0/(p.get_m()*r*sin(teta)))*force_phi - 2*(1.0/tan(teta))*teta_prime*phi_prime; //double dérivée de l'angle phi
		}
		return Vecteur({teta_pprime, phi_pprime});
	}
}
Vecteur Spherique::position(const ObjetPhysique &p) const {
	return Vecteur({r*sin(get_teta(p))*cos(get_phi(p)), r*sin(get_teta(p))*sin(get_phi(p)), r*cos(get_teta(p))});
}
